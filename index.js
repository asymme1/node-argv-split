...
function split (str, throwOnError = true) {
  ...
  if (single_quoted) {
    error('unmatched single quote', 'UNMATCHED_SINGLE', throwOnError)
  }

  if (double_quoted) {
    error('unmatched double quote', 'UNMATCHED_DOUBLE', throwOnError)
  }

  if (escaped) {
    error('unexpected end with \\', 'ESCAPED_EOF', throwOnError)
  }
...
function error (message, code, throwOnError) {
